...
export default function NodeGroupPage() {
  ...
  const [mock, setMock] = useState<number>(300);
  ...
  const params = useMemo(() => ({
    ...
    mock,
  }), [page, pageSize, sort, order, namespace, name, mock]);
      ...
      <Box sx={{ width: '100%', backgroundColor: '#f1f2f5' }}>
        <Box sx={{ width: '100%', padding: '20px', minHeight: 350, backgroundColor: 'white' }}>
          ...
          <Box sx={{ display: 'flex', gap: 1, alignItems: 'center', marginTop: 2, flexWrap: 'wrap' }}>
            ...
            <TextField size="small" label="Sort" value={sort||''} onChange={(e) => setSort(e.target.value||undefined)} placeholder="name|creationTimestamp" />
            <TextField size="small" label="Order" value={order||''} onChange={(e) => setOrder((e.target.value as any)||undefined)} placeholder="asc|desc" />
